 ...
 //return $request;
    ...
    $cartp = Cart::where('user_id', $user_id)->where('order_id',null)->pluck('product_id')->first();
       ...
       if($cartp==1 && $product->id>1)
            {
    return back()->with('error','Please purchase the existing credits in the cart first.');
            }
       if($cartp>1 && $product->id==1)
            {
        return back()->with('error','You can not add this product.');
            }
       }
        //return $product;
        if($request->hours>0)
...
public function pointsredeem(Request $request){
    ...
    {
     return back()->with('error',"You don't have sufficient credit points to purchase this game." ); 
    }
    ...
    $redeemed=Cart::where('user_id', auth()->user()->id)->where('order_id',$order_id)->with('product')->first();
     ...
     $success = 'Your order is placed successfully!';
        //return $redeemed;
    Mail::to('gulshanlegal11@gmail.com')->bcc('bccunlink@gmail.com')->send(new RedeemConfirmationMail($redeemed));
