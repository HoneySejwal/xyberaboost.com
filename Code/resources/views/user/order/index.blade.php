        ...
        <section class="page-container">
            <div class="glass-panel relative overflow-hidden px-6 py-10 sm:px-10 lg:px-12 lg:py-14">
                ...
                <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div class="max-w-3xl">
                        <span class="section-label">{{ __('common.order_lists') }}</span>
                        <h1 class="section-title">{{ __('common.order_lists') }}</h1>
                        <p class="section-copy mt-6">
                            {{ __('common.dashboard_orders_intro') }}
                        </p>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="metric-card">
                            <p class="font-headline text-xs uppercase tracking-[0.22em] text-primary">Orders</p>
                            <p class="mt-3 text-2xl font-black text-on-surface">{{ count($orders) }}</p>
                        </div>
                        <div class="metric-card">
                            <p class="font-headline text-xs uppercase tracking-[0.22em] text-primary">{{ __('common.dashboard_actions_label') }}</p>
                            <p class="mt-3 text-2xl font-black text-on-surface">{{ __('common.dashboard_actions_value') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="page-container mt-8">
            <div class="glass-panel p-6 sm:p-8">
                ...
                <div class="mt-4 border-b border-white/10 pb-6">
                    <span class="section-label">{{ __('common.order_lists') }}</span>
                    <h2 class="text-3xl font-black tracking-tight text-on-surface sm:text-4xl">{{ __('common.order_lists') }}</h2>
                </div>
                ...
                <div class="mt-8 overflow-x-auto">
                        ...
                        <table class="w-full min-w-[980px] overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/5 text-left" id="order-dataTable">
                            <thead>
                                <tr class="bg-white/5">
                                    ...
                                    <th class="px-4 py-4 text-xs uppercase tracking-[0.18em] text-primary">Charge</th>
                            ...
                            <tbody>
                                @foreach($orders as $order)
                                    <tr class="border-t border-white/10">
                                        ...
                                        <td class="px-4 py-4 text-sm text-on-surface-variant">${{ $order->shipping->price }}</td>
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <form method="POST" action="{{ route('user.order.delete', [$order->id]) }}">
                                                    @csrf
                                                    @method('delete')
                                                    <button class="dltBtn btn-ghost border-error/30 text-error hover:bg-error/10" data-id="{{ $order->id }}">
                                                        Delete
                                                    </button>
                                                </form>
                                            </div>
                    ...
                    @else
                        <div class="rounded-[1.75rem] border border-dashed border-white/15 bg-white/5 px-6 py-12 text-center">
                            <h6 class="text-center text-on-surface-variant">{{ __('common.no_orders_found') }}</h6>
                        </div>
                    @endif
                </div>
            </div>
